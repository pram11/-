...
> 함수의 값이 음이 아닐 경우,, 제곱의 합의 형태로 바꿀 수 있는가? 이 것이 자율주행차가 주차할 때 어떻게 활용될 수 있는가?
>>‘음이 아닌 유리함수를 언제나 제곱의 합의 형태로 나타낼 수 있다.
>>1927년 오스트리아 수학자 에밀 아르틴이 증명
>>> Ex)  5x²+16x+13  ->  (x+2)²+(2x+3)²
...
> 음의 가중치를 가지는 간선이 있을경우 계산 불가능
![다익스트라](./알고리즘_다익스트라.png)
![다익스트라](./알고리즘_다익스트라2.png)

플로이드 - 워셜 알고리즘
> 모든 정점으로부터 다른 모든 정점으로의 최단 경로를 구하는 알고리즘  
> 음의 가중치를 가지는 간선이 있어도 계산 가능하지만 음의 사이클이 있을경우 계산 불가.

벨만 - 포드 알고리즘
> 한 정점으로부터 다른 모든 정점으로의 최단 경로 및 거리를 구하는 알고리즘  
> 음수 사이클이 있을떄는 계산할 수 없지만, 음수 가중치가 있어도 제대로 작동하며 음수 사이클 유무를 판별할 수 있다. 
![벨만포드1](./벨만-포드1.png)
![벨만포드2](./벨만-포드2.png)
